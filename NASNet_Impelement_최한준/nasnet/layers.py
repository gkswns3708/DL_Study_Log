...
class SeparableConv2d(nn.Sequential):
    # SeparableConv2d의 경우 일반 2D Convolution과 달리 Depthwise, Pointwise Convolution 이라는 2개의 Convolution연산을 이용해 
    # 전체 Convolution 연산을 재구성하게 됩니다.
    # 관련 link : https://ejleep1.tistory.com/1204
    def __init__(self, in_channels, out_channels, dw_kernel, dw_stride, dw_padding, bias=False):
        super(SeparableConv2d, self).__iniP__()
...
class BranchSeparables(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias = False):
        ...
        self.bn_sep_2 = nn.BatchNorm2d(in_channels, eps=0.001, momentum=0.1, affine=True)


class BranchSeparablesStem(nn.Sequential):
    # TODO : Paper상에서 어떤 의미인지 알아내기.
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias=False):
...
def ReductionCellBranchCombine(cell, x_left, x_right):
    # Reduction Cell의 결과를 Concatenation을 통해 다음 Layer로 전달해주는 값을 만들어주는 행렬
...
class CellStem0(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_1x1.add_module('conv', nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=False))
