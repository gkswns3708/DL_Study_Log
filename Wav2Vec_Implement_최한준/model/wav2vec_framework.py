...
class Wav2Vec2Framework(nn.Module):
    ...
    def forward(self, input_values: torch.Tensor, lengths: torch.Tensor):
        ...
        Returns:
            tuple(
                hidden_sates (torch.Tensor) : with shape '(B, L, D2)',
                quantized_features (torch.Tensor) : with shpae '(B, L, D2)',
                perplexity (torch.Tensor) : with shape '(G, V)'
                time_make (torch.BoolTensor) : with shape '(B, L)'
            )
        """
        
        hidden_states, length = self.feature_extractor(input_values, lengths)
    ...
    def time_masking(self, hidden_states: torch.Tensor, lengths: torch.Tensor) -> tuple[torch.Tensor, torch.BoolTensor]:
        ...
        Args:
            hidden_sates (torch.Tensor): with shpae '(B, L, D)'
            lengths (torch.Tensor): with shape '(B)'

        Returns:
            tuple[torch.Tensor, torch.BoolTensor]: 
                Masked hidden states (torch Tensor) : with shape '(B, L, D)',
                Time mask (torch.BoolTensor) : with shape '(B, L)'
        """
        ...
        for batch in range(batch_size):
            ...
            for i in range(self.num_mask_time_steps):
                time_mask_indices[batch, start_time_idx_array+i] 
            
        time_mask_indices = time_mask_indices[:, :-self.num_mask_time_stpes]
